...
const PersonalScreen = () => {
  ...
  const fetchUserData = useCallback(async (showLoadingIndicator = true) => {
    ...
    try {
      ...
      setUserData({ // Fallback khi có lỗi API
        name: 'Lỗi tải hồ sơ',
        email: 'N/A',
        phone: 'N/A',
        avatar: '',
  ...
  const fetchRecipes = useCallback(async (showLoadingIndicator = true) => {
    if (showLoadingIndicator) setLoading(true);
    try {
      const params: any = {
        limit: 20,
  ...
  const renderHeader = () => (
    <View className="items-center mt-2 mb-4">
      ...
      <Text className="text-4xl font-bold text-[#88131B] mb-2">
        {userData?.name || (loading && !userData ? 'Đang tải...' : 'Chưa cập nhật tên')}
      </Text>
  ...
  if (loading && !recipes.length && !userData) { // Hiển thị loading indicator toàn màn hình ban đầu
    return (
      <ImageBackground source={backgroundImage} style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }} resizeMode="cover">
      ...
      <SafeAreaView className="flex-1">
        <FlatList
          ...
          renderItem={({ item }) => (
            ...
            >
              <Image
                source={{ uri: item.imageUrl || 'https://via.placeholder.com/150' }} // Placeholder nếu không có ảnh
                className="h-28 w-full"
                resizeMode="cover"
              />
              <View className="p-2">
                <Text className="text-sm font-bold text-black mb-1" numberOfLines={1}>{item.name}</Text>
                <Text className="text-xs text-gray-500">
                  ⏱ {item.preparationTimeMinutes ? `${item.preparationTimeMinutes} phút` : 'N/A'}
                </Text>
