...
export const AdminPostManagementScreen = () => {
  ...
  const posts = [
    {
      ...
      author: "Trung Phong",
      authorAvatar: "https://cdn.pixabay.com/photo/2021/07/03/20/06/woman-6384768_1280.jpg",
      date: "15/04/2023",
      status: "approved",
      likes: 245,
      comments: 32,
  ...
  const getStatusColor = (status) => {
    switch (status) {
      case "approved":
        return { bg: "bg-green-100", text: "text-green-600", label: "Đã duyệt" }
      case "pending":
        return { bg: "bg-yellow-100", text: "text-yellow-600", label: "Chờ duyệt" }
      case "rejected":
        return { bg: "bg-red-100", text: "text-red-600", label: "Từ chối" }
      default:
        return { bg: "bg-gray-100", text: "text-gray-600", label: "Không xác định" }
    }
  }
    ...
    return (
      <View className="bg-white rounded-xl p-4 shadow-sm mb-4">
        <View className="flex-row items-center mb-3">
          <Image source={{ uri: item.authorAvatar }} className="w-10 h-10 rounded-full mr-3" />
          ...
          <View className={`px-2 py-1 rounded-full ${statusStyle.bg}`}>
            <Text className={`text-xs ${statusStyle.text}`}>{statusStyle.label}</Text>
          </View>
        </View>
    ...
    <AdminHeader title="Quản lý bài đăng" />
      ...
      <View className="px-4 py-3">
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} className="mb-3">
          ...
          >
            <Text className={`${selectedFilter === "approved" ? "text-white" : "text-gray-700"}`}>Đã duyệt</Text>
          </TouchableOpacity>
          <TouchableOpacity
            className={`px-4 py-2 rounded-full mr-2 ${selectedFilter === "pending" ? "bg-[#941D23]" : "bg-white"}`}
            onPress={() => setSelectedFilter("pending")}
          >
            <Text className={`${selectedFilter === "pending" ? "text-white" : "text-gray-700"}`}>Chờ duyệt</Text>
          </TouchableOpacity>
          <TouchableOpacity
            className={`px-4 py-2 rounded-full mr-2 ${selectedFilter === "rejected" ? "bg-[#941D23]" : "bg-white"}`}
            onPress={() => setSelectedFilter("rejected")}
          >
            <Text className={`${selectedFilter === "rejected" ? "text-white" : "text-gray-700"}`}>Từ chối</Text>
          </TouchableOpacity>
        </ScrollView>
      </View>
