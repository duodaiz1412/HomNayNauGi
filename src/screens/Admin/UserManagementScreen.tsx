...
export const UserManagementScreen = () => {
  ...
  const renderUserItem = ({ item }) => (
    <View className="bg-white rounded-xl p-4 shadow-sm mb-4 flex-row">
      <Image source={{ uri: item.avatar }} className="w-16 h-16 rounded-full mr-3" />
      <View className="flex-1">
        <View className="flex-row items-center justify-between">
          ...
          <View
            className={`px-2 py-1 rounded-full ${
              item.role === "admin" ? "bg-purple-100" : item.status === "active" ? "bg-green-100" : "bg-red-100"
            }`}
          >
            <Text
              className={`text-xs ${
                item.role === "admin" ? "text-purple-600" : item.status === "active" ? "text-green-600" : "text-red-600"
              }`}
            >
              {item.role === "admin" ? "Admin" : item.status === "active" ? "Hoạt động" : "Không hoạt động"}
            </Text>
          </View>
        </View>
        ...
        <View className="flex-row mt-2">
          <View className="flex-row items-center mr-4">
            ...
            <Text className="text-gray-500 text-xs ml-1">{item.posts} bài viết</Text>
          </View>
          <View className="flex-row items-center">
            <Ionicons name="people-outline" size={14} color="#454442" />
            <Text className="text-gray-500 text-xs ml-1">{item.followers} người theo dõi</Text>
          </View>
        </View>
