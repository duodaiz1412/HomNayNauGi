...
export const UserDetailScreen = () => {
  ...
  const renderFavoriteItem = ({ item }) => (
    <View className="bg-white rounded-xl p-4 shadow-sm mb-4 flex-row">
      <Image source={{ uri: item.image }} className="w-20 h-20 rounded-lg mr-3" />
      <View className="flex-1">
        <Text className="font-bold text-base mb-1">{item.name}</Text>
        <Text className="text-gray-500 text-xs mb-2">Đăng bởi: {item.author}</Text>
        <Text className="text-gray-500 text-xs">{item.date}</Text>
      </View>
    </View>
  )
    ...
    <AdminHeader title="Hồ sơ người dùng"/>
      ...
      <ScrollView className="flex-1">
        ...
        <View className="bg-white p-4 shadow-sm">
          <View className="flex-row items-center">
            <Image source={{ uri: userData.avatar }} className="w-20 h-20 rounded-full mr-4" />
            <View className="flex-1">
              ...
              >
                <Text className={`text-xs ${userData.isActive ? "text-green-600" : "text-red-600"}`}>
        ...
        <View className="flex-row bg-white mt-4 border-b border-gray-200">
          ...
          >
            <Text className={`text-center font-medium ${activeTab === "posts" ? "text-[#941D23]" : "text-gray-500"}`}>
              Bài viết
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
            ...
            <>
              {userPosts.length > 0 ? (
                ...
                <View className="items-center justify-center py-8">
                  ...
                  <Text className="text-gray-500 mt-2">Không có bài viết nào</Text>
                </View>
              )}
            </>
          )}
          ...
          {activeTab === "favorites" && (
            <>
              {userFavorites.length > 0 ? (
                userFavorites.map((favorite) => renderFavoriteItem({ item: favorite }))
              ) : (
                <View className="items-center justify-center py-8">
                  <Ionicons name="heart-outline" size={48} color="#CCCCCC" />
                  <Text className="text-gray-500 mt-2">Không có món ăn yêu thích nào</Text>
                </View>
              )}
            </>
          )}
            ...
            <>
              {userActivity.length > 0 ? (
                ...
                <View className="items-center justify-center py-8">
                  ...
                  <Text className="text-gray-500 mt-2">Không có hoạt động nào gần đây</Text>
                </View>
              )}
            </>
          )}
